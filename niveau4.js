export class niveau4 extends Phaser.Scene {
    ...
    init(data) {
        ...
        if (this.entrance == "niveau5" || this.entrance == "niveau3")
    ...
    create() { // Création des éléments dès l'initialisation du jeu
        ...
        this.map1 = this.add.tilemap('niveau4');
        ...
        if (this.entrance == 'niveau3') {
            this.player = this.physics.add.sprite(544, 256, 'ninja');
            this.spawn_mob = false;
            this.longueurChaine = 146;
        }
        else if (this.entrance == 'niveau5') {
            this.player = this.physics.add.sprite(64, 96, 'ninja');
            this.spwan_mob = false;
            this.longueurChaine = 146;
        }
        else {
            this.player = this.physics.add.sprite(544, 256, 'ninja'); //812,1316
            this.lifePlayer = 6;
            this.spawn_mob = false;
            this.longueurChaine = 146;
    ...
    update() {
        ...
        if (this.lifePlayer <= 0) {
            this.lifePlayer = 6;
            //this.gameOver = true;
            this.scene.start('niveau4', { entrance: 'niveau4', lifePlayer : this.lifePlayer }) // reset
        }
       ...
       /*if (Phaser.Input.Keyboard.JustDown(this.keyE)){
        ...
        if (this.player.x > 592 && this.player.y > 224 ) { // Vers le niveau 3
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau3', { entrance: 'niveau4', lifePlayer:this.lifePlayer})
            })
        }
        if (this.player.x < 60 && this.player.y < 4*32) { // Vers le niveau 5
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau5', { entrance: 'niveau4', lifePlayer:this.lifePlayer })
            })
        }
    ...
    dropCoeur(x1, y1) {
        if ((Math.random()) < 0.2) { // 20% de taux de drop
   ...
   /* jump(poids, blocCible) {
    ...
    spawn_mobs() {
        ...
        this.mobs.create(5*32, 128, 'marcheSamurai').anims.play('mobGauche', true);
        this.mobs.create(9*32, 320, 'marcheSamurai').anims.play('mobGauche',true);
        this.mobs.create(14*32, 4*32, 'marcheSamurai').anims.play('mobGauche',true);
        this.mobs.create(17*32, 320, 'marcheSamurai').anims.play('mobGauche',true);
