export class niveau7 extends Phaser.Scene {
    constructor() {
        super("niveau7");
    ...
    init(data) {
        ...
        if (this.entrance == "niveau6" || this.entrance == "niveau8")
    ...
    preload() { // préchargement des assets
        ...
        this.load.tilemapTiledJSON('niveau7', 'tiled/level7_zelda.json'); // import fichier tiled
    ...
    create() { // Création des éléments dès l'initialisation du jeu
        ...
        this.map1 = this.add.tilemap('niveau7');
    ...
    update() {
        ...
        if (this.lifePlayer <= 0) {
            this.lifePlayer = 6;
            //this.gameOver = true;
            this.scene.start('niveau7', { entrance: 'niveau7', lifePlayer : this.lifePlayer }) // reset
        }
        ...
        if (this.player.x < 60 && this.player.y < 192 ) { // Vers le niveau 6
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau6', { entrance: 'niveau7', lifePlayer:this.lifePlayer})
            })
        }
        if (this.player.x > (18*32) && this.player.y > (5*32) && this.player.y < (8*32)) { // Vers le niveau 8
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau8', { entrance: 'niveau7', lifePlayer:this.lifePlayer })
            })
        }
   ...
   jump(poids, blocCible) {
    ...
    spawn_mobs() {
        ...
        this.mobs.create(128, 320, 'marcheSamurai').anims.play('mobGauche', true);
        this.mobs.create(18*32, 6*32, 'marcheSamurai').anims.play('mobGauche',true);
