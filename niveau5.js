export class niveau5 extends Phaser.Scene {
    constructor() {
        super("niveau5");
    ...
    init(data) {
        ...
        if (this.entrance == "niveau4" || this.entrance == "niveau6")
    ...
    preload() { // préchargement des assets
        ...
        this.load.tilemapTiledJSON('niveau5', 'tiled/level5_zelda.json'); // import fichier tiled
    ...
    create() { // Création des éléments dès l'initialisation du jeu
        ...
        this.map1 = this.add.tilemap('niveau5');
    ...
    update() {
        ...
        if (this.lifePlayer <= 0) {
            this.lifePlayer = 6;
            //this.gameOver = true;
            this.scene.start('niveau5', { entrance: 'niveau5', lifePlayer : this.lifePlayer }) // reset
        }
        ...
        if (this.player.x < 60 && this.player.y < 96 ) { // Vers le niveau 4
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau4', { entrance: 'niveau5', lifePlayer:this.lifePlayer})
            })
        }
        if (this.player.x > (18*32) && this.player.y > (8*32)) { // Vers le niveau 6
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau6', { entrance: 'niveau5', lifePlayer:this.lifePlayer })
            })
        }
   ...
   jump(poids, blocCible) {
    ...
    spawn_mobs() {
        ...
        this.mobs.create(4*32, 7*32, 'marcheSamurai').anims.play('mobGauche', true);
        this.mobs.create(19*32, 320, 'marcheSamurai').anims.play('mobGauche',true);
