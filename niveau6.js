export class niveau6 extends Phaser.Scene {
    constructor() {
        super("niveau6");
    ...
    init(data) {
        ...
        if (this.entrance == "niveau5" || this.entrance == "niveau7")
    ...
    preload() { // préchargement des assets
        ...
        this.load.tilemapTiledJSON('niveau6', 'tiled/level6_zelda.json'); // import fichier tiled
    ...
    create() { // Création des éléments dès l'initialisation du jeu
        ...
        this.map1 = this.add.tilemap('niveau6');
    ...
    update() {
        ...
        if (this.lifePlayer <= 0) {
            this.lifePlayer = 6;
            //this.gameOver = true;
            this.scene.start('niveau6', { entrance: 'niveau6', lifePlayer : this.lifePlayer }) // reset
        }
        ...
        if (this.player.x < 60 && this.player.y < 192 ) { // Vers le niveau 5
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau7', { entrance: 'niveau6', lifePlayer:this.lifePlayer})
            })
        }
        if (this.player.x > (18*32) && this.player.y > (8*32)) { // Vers le niveau 7
            this.cameras.main.fadeOut(400, 0, 0, 0);
            this.time.delayedCall(400, () => {
                this.scene.start('niveau5', { entrance: 'niveau6', lifePlayer:this.lifePlayer })
            })
        }
   ...
   jump(poids, blocCible) {
    ...
    spawn_mobs() {
        ...
        this.mobs.create(5*32, 5*32, 'marcheSamurai').anims.play('mobGauche', true);
        this.mobs.create(592, 320, 'marcheSamurai').anims.play('mobGauche',true);
